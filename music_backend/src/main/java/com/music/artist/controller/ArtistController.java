...
public class ArtistController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ArtistDto.Response> getOne(@PathVariable("id") Long id) {
        return ResponseEntity.ok(artistService.getArtistById(id));
    }

    @Operation(summary = "아티스트 정보 수정", description = "아티스트 정보를 수정합니다.")
    @PutMapping("/{id}")
    public ResponseEntity<ArtistDto.Response> update(@PathVariable("id") Long id, @RequestBody ArtistDto.Request request) {
