...
public class WebConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(jwtInterceptor)
                ...
                .excludePathPatterns(
                	"/",                    // 루트 경로 제외
                    "/index.html",          // index.html 제외
                    "/*.html",              // 모든 HTML 파일 제외
                    "/api/users/register",
                    "/api/users/send-password", // 비밀번호 찾기 API
                    "/api/auth/password-reset-request", // 프론트엔드에서 사용하는 경로
                    "/user/login/**",
                    "/user/regist/**",
                    "/user/create/**",
                    ...
                    "/uploads/musicfile/**", // 음악 파일 접근 제외
                    "/music/uploads/**",    // 새로운 업로드 경로 제외
                    "/api/admin/music/formats", // 지원 형식 조회는 토큰 없이 접근 가능
                    "/api/admin/music/download/**", // 다운로드는 토큰 없이 접근 가능
                    "/api/artists"          // 인기 아티스트 조회는 토큰 없이 접근 가능
                ); // 적용 제외 경로
    }
    @Override
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        ...
        registry.addResourceHandler("/uploads/musicfile/**")
                .addResourceLocations("file:" + absoluteUploadPath)
                .setCachePeriod(3600); // 1시간 캐시
        
        // SPA를 위한 정적 리소스 핸들러 - API 경로는 제외
