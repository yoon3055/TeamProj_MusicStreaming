...
public class SongController {
    ...
    @Operation(summary = "아티스트별 노래 목록 조회", description = "특정 아티스트가 등록한 모든 노래를 조회합니다.")
    public ResponseEntity<List<Song>> getSongsByArtist(@PathVariable("artistId") Long artistId) {
        try {
            log.info("=== [SONG_CONTROLLER] GET /api/songs/by-artist/{} 요청 받음 ===", artistId);
            List<Song> songs = songService.getSongsByArtistId(artistId);
            log.info("아티스트별 노래 목록 조회 성공: artistId={}, 결과 {}개", artistId, songs.size());
            return ResponseEntity.ok(songs);
        } catch (Exception e) {
            log.error("아티스트별 노래 목록 조회 중 오류 발생: artistId={}", artistId, e);
            throw e;
        }
    }

    @GetMapping("/recent")
    @Operation(summary = "최근 업로드된 노래 조회", description = "지정된 일수 내에 업로드된 최신 노래들을 조회합니다.")
    public ResponseEntity<List<Song>> getRecentSongs(
            @RequestParam(value = "days", defaultValue = "7") int days) {
        try {
            log.info("=== [SONG_CONTROLLER] GET /api/songs/recent 요청 받음 ===");
            log.info("최근 {}일간 업로드된 노래 조회", days);
            List<Song> songs = songService.getRecentSongs(days);
            log.info("최근 노래 조회 성공: {}개", songs.size());
            return ResponseEntity.ok(songs);
        } catch (Exception e) {
            log.error("최근 노래 조회 중 오류 발생: days={}", days, e);
            throw e;
        }
    }
}
